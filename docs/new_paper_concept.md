...
- **S-NeRF型 信頼度融合での多パノ統合＝実装済み**（2026-06-28）。各視点の意味equirectから
  壁/物体を抽出し、GPS(ENU)で共有ワールドへ配置（解析的姿勢が北整列なのでヨー推定不要）。
  複数視点が見た同じ壁/物体を近接クラスタで束ね、**confidence=支持視点数/視点数**を付与
  （`_fuse_colliders`）。`support_views≥2` を multi_view_confirmed として集計し、debug メッシュは
  信頼度で不透明度を変える。→ コア主張1（信頼度加重融合, S-NeRF Ĉ の意味コライダー版）を実装。
- 次：3DGS化（見た目の写真品質）。
